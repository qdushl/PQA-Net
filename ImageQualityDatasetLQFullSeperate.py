...
class ImageQualityDatasetLQFull(Dataset):
    def __init__(self, csv_file_dist, csv_file_mos, root_dir_dist, root_dir_mos, enable_dist, transform=None,
                 num_plane=6):
        ...
        if self.enable_dist:
            ...
            self.frame = pd.read_csv(img_ori, header=None)
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_dist:
            img_name = os.path.join(self.root_dir_dist, self.frame.iloc[idx, 0])
            image = Image.open(img_name)
        else:
            img_set = self.frame.iloc[idx, 0].rsplit('_', 1)
            img_set_name = img_set[0]  # bag_gsigma_2_tsigma_8_view_1
            ...
            for i in range(self.num_plane):
                ...
                if i:
                    ...
                    if self.transform:
                        ...
                        images_arr = image_arr
            images = transformtensor(images_arr)
        score = self.frame.iloc[idx, 1]
        if self.enable_dist:
            disttype = self.frame.iloc[idx, 2]  # distortion type
            sample = {'image': image, 'score': score, 'disttype': disttype, 'image_name': img_name, 'patch_num': image.shape[0]}
        else:
            sample = {'image': images, 'score': score, 'image_name': img_name, 'patch_num': images.shape[0]}
